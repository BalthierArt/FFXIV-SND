...
public unsafe class EX4_Escelons_Fall_Braindead DPS in: SplatoonScript
{
    ...
    public override void OnSettingsDraw()
    {
        ...
        // Ensure mutual exclusivity and require one to be checked
        if(dpsChanged)
        {
            if(C.IsDPS)
            {
            ...
            {
                C.IsDPS = true; // Force DPS if both would be unchecked
            }
        }
        if(supportChanged)
        {
            ...
            {
                C.IsSupport = true; // Force Support if both would be unchecked
            }
        }
    ...
    {
        // DPS starts IN, Support starts OUT
        bool startClose = C.IsDPS;
        if(this.AdjustPhase)
        {
            if(THShockTargeted)
            {
                startClose = !C.IsDPS; // Reverse for shock phase if targeted
            }
            else
            {
                startClose = C.IsDPS;  // Normal behavior
            }
        }
        return [startClose, !startClose, startClose, !startClose];
    }
    ...
    {
        public bool IsDPS = true;    // Default to DPS
